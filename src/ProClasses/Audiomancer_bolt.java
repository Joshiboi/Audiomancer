...
public class Audiomancer_bolt extends JPanel {
	private static final long serialVersionUID = 12314255L;
	...
	public Audiomancer_bolt(TileMap tm, int _x, int _y, int _xSpd) {
		tileMap = tm;
		x = _x;
		y = _y;
		xSpd = _xSpd;
	...
	public void draw(Graphics g, int current)
    {
        ...
        width = bolt[animFrames-1].getWidth(null);
        height = bolt[animFrames-1].getHeight(null);
        
        tempx = checkLRCollisions(tempx, currCol);
        x=tempx;
        
        g2d.drawImage(bolt[current],x+tileMap.getX(),y+tileMap.getY(),this);
    }
